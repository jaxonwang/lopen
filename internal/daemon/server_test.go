...
// file) and returns it plus a connected client socket path.
func newTestServer(t *testing.T) (*Server, string, string) {
	...
	tun := &Tunnel{Host: cfg.Hosts[0], LocalSock: sock, Log: srv.Log}
	srv.tunnels["devbox"] = tun
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
	go srv.acceptLoop(ctx, ln, tun)
	return srv, sock, openLog
}

func TestInlineFileEndToEnd(t *testing.T) {
	srv, sock, openLog := newTestServer(t)
	...
	err := client.Open(src, client.Options{
		Socket: sock, Label: "devbox", Wait: true, Retry: time.Second,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInlineDirEndToEnd(t *testing.T) {
	...
	if err := client.Open(src, client.Options{Socket: sock, Label: "devbox", Wait: true}); err != nil {
		t.Fatal(err)
	}
	mirrored := filepath.Join(srv.Cfg.MirrorDir, "devbox", src)
...
func TestLocallyModifiedGuard(t *testing.T) {
	...
	err := client.Open(src, client.Options{Socket: sock, Label: "devbox", Wait: true})
	if err == nil {
	...
	// --force overrides.
	if err := client.Open(src, client.Options{Socket: sock, Label: "devbox", Wait: true, Force: true}); err != nil {
		t.Fatal(err)
	}
...
func TestInlineOriginSpoofRefused(t *testing.T) {
	_, sock, _ := newTestServer(t) // configures only "devbox"
	conn, err := net.Dial("unix", sock)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRejectsRootPath(t *testing.T) {
	...
	req := &protocol.Request{
		...
		Path:   "/", Mode: protocol.ModeInline, Wait: true,
	}
	if err := protocol.WriteRequest(conn, req); err != nil {
		t.Fatal(err)
	}
